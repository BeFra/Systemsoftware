...
MODULE_DESCRIPTION("A mutex module");
MODULE_SUPPORTED_DEVICE("none");

#define DRIVERNAME "OPEN_ONCE"
...
static int open(struct inode *devicefile, struct file *instanz) {
	...
	while(1) {
		retMutex = mutex_trylock(&m);
		if(retMutex != 1) {
			printk(DRIVERNAME ": mutex lock not available. Wait.\n");
			msleep(200);
		} else {
			break;
		}
	...
	printk(DRIVERNAME": mutex lock available.\n");	
	ssleep(3);
